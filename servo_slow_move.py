...
用法:
  # 移动到指定刻度（默认慢速）
  py -3.11 servo_slow_move.py 680

  # 指定速度等级 (1=很慢, 2=慢, 3=中, 4=快)
  py -3.11 servo_slow_move.py 680 --speed 2

  # 往复循环测试（600 <-> 750）
  py -3.11 servo_slow_move.py --loop

  # 作为模块导入
  from servo_slow_move import ServoSlowMover
...
def main():
    parser = argparse.ArgumentParser(description="SCS0009 舵机慢速位置控制")
    ...
    try:
        ...
        if args.loop:
            ...
            # 无参数时显示当前状态
            print("\n用法:")
            print(f"  {__file__} 680          # 移动到刻度 680")
            print(f"  {__file__} 680 --speed 1  # 很慢速度移动")
