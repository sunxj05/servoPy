...
换算公式: 刻度 u = 500 + 弧度 rad × (1000 / 2π)
          刻度 610 ↔ 39.6°，刻度 700 ↔ 72°

本脚本控制舵机 ID=1，活动区间限制为 610-700 刻度。

用法:
  # 移动到指定刻度（610-700 范围内，默认慢速）
  py -3.11 servo_slow_move_id1.py 650

  # 指定速度等级 (1=很慢, 2=慢, 3=中, 4=快)
  py -3.11 servo_slow_move_id1.py 650 --speed 2

  # 往复循环测试（610 <-> 700）
  py -3.11 servo_slow_move_id1.py --loop

  # 作为模块导入
  from servo_slow_move_id1 import ServoSlowMover
...
def main():
    parser = argparse.ArgumentParser(description="SCS0009 舵机慢速位置控制（ID=1）")
    ...
    try:
        ...
        if args.loop:
            ...
            # 无参数时显示当前状态
            print("\n用法:")
            print(f"  {__file__} 650          # 移动到刻度 650")
            print(f"  {__file__} 650 --speed 1  # 很慢速度移动")
